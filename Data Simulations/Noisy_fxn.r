...
# 1. Simulating Noisy counts 
simulate_noisy_counts <- function(
    true_counts = true_counts_res[[1]], 
    meta_cell = true_counts_res[[3]], 
    gene_len_pool = gene_len_pool, 
    ngenes, 
    protocol = "nonUMI", 
    alpha_mean = 0.1, 
    alpha_sd = 0.05, 
    ...
    {  
        ...
        return(observed_counts[["counts"]])  
    }

Noisy1 <- simulate_noisy_counts()
Noisy2 <- simulate_noisy_counts()
...
observed_counts <- True2ObservedCounts(
    ...
    protocol="UMI",  
    alpha_mean = alpha_mean ,  
    alpha_sd = alpha_mean*0.25, 
    gene_len=gene_len,
    depth_mean = depht_mean, nPCR1 = 14, 
    depth_sd = depht_mean*0.25)
